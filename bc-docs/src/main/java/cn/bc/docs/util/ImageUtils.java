...
public class ImageUtils {
   ...
   * @throws IOException IO异常
   */
  public static BufferedImage zoom(InputStream inputStream, double zoomRatio, String extention) throws IOException {
    return zoom(ImageIO.read(inputStream), zoomRatio, extention);
  }

  public static BufferedImage zoom(BufferedImage image, double zoomRatio, String extention) throws IOException {
    ...
    BufferedImage bufferedImage = new BufferedImage(width, height, getTargetImgType(image, extention));
  ...
  public static int getTargetImgType(BufferedImage image, String extension) {
  ...
  public static BufferedImage combineHorizontal(Imager[] imagers) {
    ...
    for (Imager imager : imagers) {
      width += imager.getScaleWidth();
      height = Math.max(height, imager.getScaleHeight());
    ...
    int[] ImageArrayOne;
    for (Imager imager : imagers) {
      width = imager.getScaleRealWidth();// 原始图像宽度
      height = imager.getScaleRealHeight();// 原始图像高度
      startX += imager.getLeft();// 左间隙
      startY = imager.getTop();// 上间隙

      // 原图的数据
      ImageArrayOne = new int[width * height];
      ImageArrayOne = imager.getScaleImage().getRGB(0, 0, width, height, ImageArrayOne, 0, width);
  ...
  public static BufferedImage combineHorizontal(BufferedImage[] images) throws IOException {
   ...
   * @throws IOException
   */
  public static BufferedImage combineHorizontal(BufferedImage[] images, int gap) throws IOException {
   ...
   *                  如果要控制图片的缩放，如"1:5 6:1.2,1"，表示第一张图片合并前会先放大到1.2倍的像素大小然后再参与合并。
   * @return
   * @throws IOException
   */
      ...
      for (int j = 0; j < h_imagers.length; j++) {
        ...
        if (_gaps != null && _gaps.length > 0) {
          if (_gaps.length == 1) {// 统一间隙
            ...
            throw new CoreException("不支持的间隙配置：" + h_cfg[1]);
          }
        }

        // 构建图片合并对象
        h_imagers[j] = new Imager(images[c + j], gaps[0], gaps[1], gaps[2], gaps[3], h_cfg.length > 2 ? Double.parseDouble(h_cfg[2]) : 1);
      }
      c += h_imagers.length;
