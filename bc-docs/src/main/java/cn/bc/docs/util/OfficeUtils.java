...
public class OfficeUtils {
   ...
   * </p>
   *
   * @param fromFile 原始文档的相对路径，开头不要字符"/"，父路径由Attach.DATA_REAL_PATH 的值决定
   * @param toType   转换后文档的格式，如"pdf"
   * @param cache    是否使用缓存值，为true则会先检测"convert/[fromFileName].[toType]"是否存在，
   *                 存在就直接返回此文件不作任何转换
   */
