...
public interface AttachService extends CrudService<Attach> {
   ...
   *
   * @param fromPtype      原附件的ptype
   * @param fromPuid       原附件的puid
   * @param toPtype        新附件的ptype
   * @param toPuid         新附件的puid
   * @param keepAuthorInfo 是否保留源附件的创建信息不变，设为false将创建时间和创建人设为当前用户的信息
   * @return 复制出来的附件列表
   */
