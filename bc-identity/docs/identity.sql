...
CREATE OR REPLACE FUNCTION identity_find_actor_belong_ids(actor_id int)
	...
	BEGIN
		return query 
		select r.master_id from bc_identity_actor_relation r
...
CREATE OR REPLACE FUNCTION identity_find_actor_role_ids(actor_id int)
	...
	BEGIN
		return query 
		select ra.aid, ra.rid from bc_identity_role_actor ra 
			inner join (
				select a.id from bc_identity_actor a where id = $1
				union all 
...
CREATE OR REPLACE FUNCTION identity_find_actor_role_ids(actor_code character varying)
	...
	BEGIN
		...
		select ra.aid, ra.rid from bc_identity_role_actor ra 
			inner join (
				select a.id from bc_identity_actor a where code = $1
				union all 
				select identity_find_actor_ancestor_ids($1)
			) a on a.id = ra.aid;
	END;
