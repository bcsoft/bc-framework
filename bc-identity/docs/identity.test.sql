...
-- 用户直属的单位或部门、所在的岗位
select u.id,u.name,o.id,o.type_,o.name from bc_identity_actor o
...
-- 查找直接配有指定角色的用户
select r.name 角色,u.code 帐号,u.name 姓名 from bc_identity_actor u
...
-- 查找直接配有指定岗位的用户
select g.name 岗位,u.code 帐号,u.name 姓名 from bc_identity_actor u
...
-- 查找ACL
select d.doc_id,d.doc_name from bc_acl_doc d
