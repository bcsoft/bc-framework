...
CREATE TABLE BC_TEMPLATE(
	...
	CONTENT VARCHAR(255),
	INNER_ INTEGER NOT NULL DEFAULT 1,
	FILE_DATE TIMESTAMP NOT NULL,
  AUTHOR_ID INTEGER NOT NULL,
  MODIFIER_ID INTEGER ,
  MODIFIED_DATE TIMESTAMP,
...
COMMENT ON COLUMN BC_TEMPLATE.TYPE_ IS '类型：1-Excel文件、2-Word文件、3-文本文件、4-Html文件、5-其它文件';
COMMENT ON COLUMN BC_TEMPLATE.CODE IS '编码：全局唯一';
COMMENT ON COLUMN BC_TEMPLATE.NAME IS '模板名称';
COMMENT ON COLUMN BC_TEMPLATE.TEMPLATE_FILE_NAME IS '模板文件';
COMMENT ON COLUMN BC_TEMPLATE.CONTENT IS '模板内容：文本和Html类型显示模板内容';
COMMENT ON COLUMN BC_TEMPLATE.INNER_ IS '内置：0-是、1-否，默认否';
