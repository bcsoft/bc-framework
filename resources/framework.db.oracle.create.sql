...
-- 任务调度：任务 
CREATE TABLE BC_SD_CFG_JOB (
    ID number(19) NOT NULL,
    NAME varchar2(255) NOT NULL,
    BEAN varchar2(255) NOT NULL,
    METHOD varchar2(255) NOT NULL,
    SIE number(1) NOT NULL,
    DESC_ varchar2(1000),
    PRIMARY KEY (ID)
);
COMMENT ON TABLE BC_SD_CFG_JOB IS '任务调度：任务';
COMMENT ON COLUMN BC_SD_CFG_JOB.NAME IS '名称';
COMMENT ON COLUMN BC_SD_CFG_JOB.BEAN IS '要调用的SpringBean名';
COMMENT ON COLUMN BC_SD_CFG_JOB.METHOD IS '要调用的SpringBean方法名';
COMMENT ON COLUMN BC_SD_CFG_JOB.SIE IS '发现异常是否终止继续调度:0-否,1-是';
COMMENT ON COLUMN BC_SD_CFG_JOB.DESC_ IS '备注';

-- 任务调度：触发器
CREATE TABLE BC_SD_CFG_TRIGGER (
    ID number(19) NOT NULL,
    STATUS_ number(1)  NOT NULL,
    NAME varchar2(255) NOT NULL,
...
COMMENT ON TABLE BC_SD_CFG_TRIGGER IS '任务调度：触发器';
COMMENT ON COLUMN BC_SD_CFG_TRIGGER.STATUS_ IS '状态：1-启用中,0-已禁用';
COMMENT ON COLUMN BC_SD_CFG_TRIGGER.NAME IS '名称';
COMMENT ON COLUMN BC_SD_CFG_TRIGGER.CRON IS '表达式';
COMMENT ON COLUMN BC_SD_CFG_TRIGGER.ORDER_ IS '排序好';
COMMENT ON COLUMN BC_SD_CFG_TRIGGER.DESC_ IS '备注';
COMMENT ON COLUMN BC_SD_CFG_TRIGGER.JOB_ID IS '任务ID';
ALTER TABLE BC_SD_CFG_TRIGGER ADD CONSTRAINT BCFK_TRIGGER_JOB FOREIGN KEY (JOB_ID) 
	REFERENCES BC_SD_CFG_JOB (ID);
...
COMMENT ON TABLE BC_SD_LOG IS '任务调度：日志';
