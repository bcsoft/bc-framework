...
DECLARE
	...
	pfname varchar(4000);
	t integer := 0;
	curChilden cursor FOR select a.id id,a.type_ type_ from bc_identity_actor a inner join bc_identity_actor_relation r on r.follower_id = a.id 
		where r.type_=0 and r.master_id=pid order by a.order_;
	curTops cursor FOR select a.id id from bc_identity_actor a where a.type_=1 and not exists 
...
DECLARE
	...
	pfname varchar(4000);
	t integer := 0;
	curChilden cursor for select r.id id,r.type_ type_ from bc_identity_resource r where r.belong = pid order by r.order_;
	curTops cursor for select r.id id from bc_identity_resource r where COALESCE(r.belong,0) = 0 order by r.order_;
BEGIN
  	if pid > 0 then
		...
		FOR child IN curChilden LOOP
			update bc_identity_resource r set r.pname=pfname where r.id=child.id;
