...
ALTER TABLE BC_IDENTITY_ACTOR_HISTORY ADD INDEX BCIDX_ACTORHISTORY_ACTORID (ACTOR_ID)
    ,ADD INDEX BCIDX_ACTORHISTORY_UPPERID (UPPER_ID)
...
ALTER TABLE BC_BULLETIN ADD CONSTRAINT BCFK_BULLETIN_ISSUER FOREIGN KEY (ISSUER_ID) 
	REFERENCES BC_IDENTITY_ACTOR (ID);
ALTER TABLE BC_BULLETIN ADD INDEX BCIDX_BULLETIN_SEARCH (SCOPE,AUTHOR_UNIT_ID,STATUS_);
ALTER TABLE BC_BULLETIN ADD INDEX BCIDX_BULLETIN_ARCHIVE (SCOPE,AUTHOR_UNIT_ID,STATUS_,FILE_YEAR,FILE_MONTH,FILE_DAY);
...
CREATE TABLE BC_DOCS_ATTACH_HISTORY (
    ...
    AUTHOR_DEPART_ID int COMMENT '处理人所在部门ID，如果用户直接隶属于单位，则为null',
    AUTHOR_DEPART_NAME varchar(255) COMMENT '处理人所在部门名称，如果用户直接隶属于单位，则为null',
    AUTHOR_UNIT_ID int NOT NULL COMMENT '处理人所在单位ID',
    AUTHOR_UNIT_NAME varchar(255) NOT NULL COMMENT '处理人所在单位名称',
    MODIFIER_ID int COMMENT '最后修改人ID',
    MODIFIER_NAME varchar(255) COMMENT '最后修改人名称',
    MODIFIED_DATE datetime COMMENT '最后修改时间',
...
ALTER TABLE BC_FEEDBACK ADD CONSTRAINT BCFK_FEEDBACK_AUTHOR FOREIGN KEY (AUTHOR_ID) 
	REFERENCES BC_IDENTITY_ACTOR (ID);
ALTER TABLE BC_FEEDBACK ADD INDEX BCIDX_FEEDBACK_SEARCH (AUTHOR_UNIT_ID,STATUS_);
ALTER TABLE BC_FEEDBACK ADD INDEX BCIDX_FEEDBACK_ARCHIVE (AUTHOR_UNIT_ID,STATUS_,FILE_YEAR,FILE_MONTH,FILE_DAY);
