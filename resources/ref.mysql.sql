...
explain select m.*
        ...
        where m.type_ = 2 and m.id not in
                              (select f.id
                               from
                               BC_IDENTITY_ACTOR f inner join BC_IDENTITY_ACTOR_RELATION ar on f.id = ar.follower_id
                               where f.type_ = 2 and ar.type_ = 0);

-- 获取单位的下属单位或部门信息
explain select m1.name, f1.*
        from BC_IDENTITY_ACTOR f1
...
explain select f1.name, m1.*
        ...
        where m1.type_ = 2 and ar1.type_ = 0 and f1.id = (select f.id
                                                          from BC_IDENTITY_ACTOR f
                                                          where f.name = '信息化项目小组')
        order by m1.code;

-- 使用in在mysql中会导致全表查询
explain select f.*
        from BC_IDENTITY_ACTOR f
        where f.type_ in (2, 3);
