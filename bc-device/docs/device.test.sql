...
insert into bc_device (
  ...
  select NEXTVAL('hibernate_sequence'), 'D' || NEXTVAL('hibernate_sequence'), 1, 'D182.01', 'D182', '打印机', '打印工作表',
      date '2013-11-29', '7129513040002281', null, now(), (select id
...
insert into bc_device_event (id, device_id, type_, trigger_time)
  select NEXTVAL('hibernate_sequence'), (select id
                                         from bc_device
                                         where code = 'A15.01'), '自定义事件', now()
  from bc_dual
  where not exists(select 0
                   from bc_device_event
...
-- 插入新事件数据
insert into bc_device_event_new (id) (select id
                                      from bc_device_event
                                      where type_ = '打印事件')