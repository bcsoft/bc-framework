...
CREATE OR REPLACE view view_bc_email_to_receiver_info as
	select tt.id, array_to_string(array_agg(tt.type_), ',') as type_str, array_to_string(array_agg(tt.info), ';') as receiver_str
	from (
		select t.pid as id, t.type_,
			array_to_string(array_agg(distinct (case when t.upper_id is null then a.name else upper.name end)), ',') as info
			from bc_email_to t
