...
public class MixConditionTest {
  static final String ALIAS = null;//"alias_";

  @Test
  public void alias_and1_orderBy() {
    AndCondition and = new AndCondition();
    and.add(new EqualsCondition("key1", "value1")).add(
      new OrderCondition("key", Direction.Asc));
    Assert.assertEquals("alias_.key1 = ? order by alias_.key asc", and.getExpression("alias_"));
  ...
  public void and1_orderBy() {
    ...
    and.add(new EqualsCondition("key1", "value1")).add(
      new OrderCondition("key", Direction.Asc));
    Assert.assertEquals("key1 = ? order by key asc", and.getExpression(ALIAS));
    Assert.assertNotNull(and.getValues());
    Assert.assertTrue(and.getValues().size() == 1);
    Assert.assertEquals("value1", and.getValues().get(0));
  }
  ...
  public void and_or_orderBy() {
    ...
    and.add(or);
    System.out.println(and.getExpression(ALIAS));
    Assert.assertEquals(
