...
create or replace function workday__calculate_default_working_days(from_date           date, to_date date,
                                                                   workdays_every_week int default 5)
  returns int as
$BODY$
/**
 * 获取指定日期段对应的默认工作日数
...
begin
  ...
  select -- (结束日期所在周的周末 - 开始日期对上一周的周末) / 7 * 每周默认工作日数
...
create or replace function workday__calculate_real_working_days(from_date           date, to_date date,
                                                                workdays_every_week int default 5)
  returns int as
$BODY$
/**
    ...
    + coalesce((
                 select sum((
                              case when w.dayoff
                                ...
                                w.to_date - w.from_date + 1 -
                                workday__calculate_default_working_days(w.from_date, w.to_date)
                              end))
                 from (select *
                       from BC_WORKDAY w0
                       where w0.from_date >= $1 and w0.to_date <= $2) w
               ), 0) into _days;
  --raise info 'days=%', _days;
