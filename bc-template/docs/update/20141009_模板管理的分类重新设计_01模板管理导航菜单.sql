...
delete from BC_IDENTITY_ROLE_RESOURCE where sid in
	(select id from BC_IDENTITY_RESOURCE where name in ('模板管理','模板分类','模板配置','模板格式','模板参数'));
delete from BC_IDENTITY_RESOURCE where name in ('模板管理','模板分类','模板配置','模板格式','模板参数');
-- 角色
delete from BC_IDENTITY_ROLE_ACTOR where rid in
	(select id from BC_IDENTITY_ROLE where code like 'BC_TEMPLATE_%');
delete from BC_IDENTITY_ROLE where code like 'BC_TEMPLATE_%';
-- 岗位
delete from BC_IDENTITY_ACTOR_RELATION where
	FOLLOWER_ID in (
		select id from BC_IDENTITY_ACTOR
...
delete from BC_IDENTITY_ROLE_ACTOR where aid in (
	select id from BC_IDENTITY_ACTOR where name in ('模板管理岗','模板维护岗')
);
delete from BC_IDENTITY_ACTOR where name in('模板管理岗','模板维护岗');

-- ACL
delete from bc_acl_actor
	where pid in (
		select aa.pid from bc_acl_actor aa
		...
		select aa.aid from bc_acl_actor aa
			inner join bc_acl_doc d on aa.pid = d.id
			inner join bc_identity_actor a on a.id = aa.aid
			where d.doc_name='模板分类' and d.doc_type='Category'
			and a.code in ('ghy', 'jane', 'wing', 'qiuting', 'fei')
...
insert into BC_IDENTITY_ROLE (ID,STATUS_,INNER_,TYPE_,ORDER_,CODE,NAME)
	...
	from BC_DUAL
	where not exists (select 0 from BC_IDENTITY_ROLE where CODE='BC_TEMPLATE_CONFIG_MANAGE');
-- 插入模板配置管理角色包含模板格式资源
...
insert into BC_IDENTITY_ROLE (ID,STATUS_,INNER_,TYPE_,ORDER_,CODE,NAME) 
	...
	from BC_DUAL
	where not exists (select 0 from BC_IDENTITY_ROLE where CODE='BC_TEMPLATE_CONFIG_READ');
-- 插入模板配置查阅角色包含模板格式资源
insert into BC_IDENTITY_ROLE_RESOURCE (RID,SID) 
	select r.id,m.id 
	from BC_IDENTITY_ROLE r,BC_IDENTITY_RESOURCE m 
...
insert into BC_IDENTITY_ROLE (ID,STATUS_,INNER_,TYPE_,ORDER_,CODE,NAME)
	...
	from BC_DUAL
	where not exists (select 0 from BC_IDENTITY_ROLE where CODE='BC_TEMPLATE_FORMAT_MANAGE');
-- 插入模板格式查阅角色
insert into BC_IDENTITY_ROLE (ID,STATUS_,INNER_,TYPE_,ORDER_,CODE,NAME)
	select NEXTVAL('CORE_SEQUENCE'), 0, false,  0,'0009', 'BC_TEMPLATE_FORMAT_READ','模板格式查阅'
	from BC_DUAL
	where not exists (select 0 from BC_IDENTITY_ROLE where CODE='BC_TEMPLATE_FORMAT_READ');
...
insert into BC_IDENTITY_ROLE (ID,STATUS_,INNER_,TYPE_,ORDER_,CODE,NAME)
	...
	from BC_DUAL
	where not exists (select 0 from BC_IDENTITY_ROLE where CODE='BC_TEMPLATE_PARAM_MANAGE');
-- 插入模板参数查阅角色
insert into BC_IDENTITY_ROLE (ID,STATUS_,INNER_,TYPE_,ORDER_,CODE,NAME)
	select NEXTVAL('CORE_SEQUENCE'), 0, false,  0,'0009', 'BC_TEMPLATE_PARAM_READ','模板参数查阅'
	from BC_DUAL
	where not exists (select 0 from BC_IDENTITY_ROLE where CODE='BC_TEMPLATE_PARAM_READ');
...
insert into BC_IDENTITY_ACTOR (ID,UID_,STATUS_,INNER_,TYPE_,CODE, NAME, ORDER_,PCODE,PNAME)
	...
	, 'BC_TEMPLATE_MANAGE_GROUP','模板管理岗', '9930','[1]baochengzongbu','宝城'
	from BC_DUAL
	where not exists (select 0 from BC_IDENTITY_ACTOR where CODE='BC_TEMPLATE_MANAGE_GROUP');
...
insert into BC_IDENTITY_ACTOR (ID,UID_,STATUS_,INNER_,TYPE_,CODE, NAME, ORDER_,PCODE,PNAME)
	select NEXTVAL('CORE_SEQUENCE'),'group.init.'||NEXTVAL('CORE_SEQUENCE'), 0, false, 3
	, 'BC_TEMPLATE_MAINTAIN_GROUP','模板维护岗', '9931','[1]baochengzongbu','宝城'
	from BC_DUAL
	where not exists (select 0 from BC_IDENTITY_ACTOR where CODE='BC_TEMPLATE_MAINTAIN_GROUP');
insert into BC_IDENTITY_ACTOR_RELATION (TYPE_,MASTER_ID,FOLLOWER_ID)
    select 0,am.id,af.id
    from BC_IDENTITY_ACTOR am,BC_IDENTITY_ACTOR af
    where am.CODE='baochengzongbu'
...
insert into bc_acl_doc (id, doc_id, doc_type, doc_name, author_id, file_date, modifier_id, modified_date)
	select NEXTVAL('CORE_SEQUENCE'), c.id::text, 'Category', c.name_, a.id, now(), a.id, now()
		...
		and a.code='admin'
		and not exists (
			select 0 from bc_acl_doc where name_='模板分类' and doc_type='Category'
		);
insert into bc_acl_actor (pid, aid, role, order_, modifier_id, modified_date)
	select d.id, a.id, '10', 0,
		...
		and a.code in ('ghy', 'jane', 'wing', 'qiuting', 'fei')
		and not exists (
			select 0 from bc_acl_actor where pid = d.id and aid in (a.id)
		);