...
public interface TemplateService extends CrudService<Template> {
   ...
   * 根据编码获取模板对象
   *
   * @param code 如果含字符":"，则进行分拆，前面部分为编码，后面部分为版本号，如果没有字符":"，将获取当前状态为正常的版本
   * @return 指定编码的模板对象
   */
   ...
   * @param code
   * @return 指定编码的模板对象
   * @throws CoreException 模板不是字符类型时
   */
   ...
   *
   * @param code 模板的编码
   * @throws CoreException 模板不是字符串类型时
   */
   ...
   *
   * @param code 模板的编码
   * @param out  模板格式化后保存到的流
   * @throws CoreException 模板不是xml格式的Word文档时
   */
   ...
   *
   * @param id           模板id
   * @param mapFormatSql 格式化此模板参数详细配置sql中的占位符 格式为{占位名称:需要替换的值}
   * @return
   */
   ...
   * @param code           模板编码 必填
   * @param args           替换参数，注意：当模板带有参数时，若KEY相同，则会覆盖传入的参数 可输入 null值
   * @param ptype          模块类型 必填
   * @param puid           模块uid 必填
   * @param author         创建附件的作者，可以输入null 当为null值时，则查找SystemContextHolder中的userhistory
   *                       当没SystemContextHolder时，默认admin
   * @param formatParamSql 格式化模板参数sql的替换值
   * @return
   */
